...
public class AerodynamicSurface : ScriptableObject
{
    ...
    public AerodynamicSurfaceForces CalculateForces(bool forceReturn, Transform surfaceTransform, Vector3 worldAirVelocity, 
        float airDensity, Vector3 relativePosition, float currentFlapAngle)
    {
        ...
        float deltaLift = AerodynamicUtils.DeltaLift(correctedLiftSlope, AerodynamicUtils.Theta(flapFraction), currentFlapAngle);
        ...
        float correctedStallAngleHigh = correctedZeroLiftAoA + AerodynamicUtils.MaxHigh(correctedLiftSlope, this, zeroLiftAoaBase, deltaLift) / correctedLiftSlope;
        float correctedStallAngleLow = correctedZeroLiftAoA + AerodynamicUtils.MaxLow(correctedLiftSlope, this, zeroLiftAoaBase, deltaLift) / correctedLiftSlope;

        // air velocity relative to the surface's coordinate system.
        Vector3 airVelocity = AerodynamicUtils.AirVelocity(surfaceTransform.InverseTransformDirection(worldAirVelocity));
        ...
        Vector3 aerodynamicCoefficients = CalculateCoefficients(angleOfAttack, correctedLiftSlope, correctedZeroLiftAoA,
        ...
        float correctedStallAngleHigh, float correctedStallAngleLow, float currentFlapAngle)
    {
        ...
        float paddedStallAngleHigh = AerodynamicUtils.PaddedStallAngleHigh(correctedStallAngleHigh, currentFlapAngle);
        ...
        {
            aerodynamicCoefficients = CalculateCoefficientsAtLowAoA(angleOfAttack, correctedLiftSlope, correctedZeroLiftAoA);
        }
        else
        {
            ...
            else
            {
                ...
                Vector3 aerodynamicCoefficientsLow = angleOfAttackVsStallHigh
                    ? CalculateCoefficientsAtLowAoA(correctedStallAngleHigh, correctedLiftSlope, correctedZeroLiftAoA)
                    : CalculateCoefficientsAtLowAoA(correctedStallAngleLow, correctedLiftSlope, correctedZeroLiftAoA);
                Vector3 aerodynamicCoefficientsStall = angleOfAttackVsStallHigh
                    ? CalculateCoefficientsAtStall(paddedStallAngleHigh, correctedLiftSlope, correctedZeroLiftAoA,
                        correctedStallAngleHigh, correctedStallAngleLow, currentFlapAngle)
                    : CalculateCoefficientsAtStall(paddedStallAngleLow, correctedLiftSlope, correctedZeroLiftAoA, correctedStallAngleHigh,
    ...
    private Vector3 CalculateCoefficientsAtLowAoA(float angleOfAttack, float correctedLiftSlope, float correctedZeroLiftAoA)
    {
        float liftCoefficient = AerodynamicUtils.LiftC(correctedLiftSlope, angleOfAttack, correctedZeroLiftAoA);
        float effectiveAngle = angleOfAttack - correctedZeroLiftAoA - (liftCoefficient / (AerodynamicUtils.pi * aspectRatio));
    ...
    private Vector3 CalculateCoefficientsAtStall(float angleOfAttack, float correctedLiftSlope, float correctedZeroLiftAoA, 
        float correctedStallAngleHigh, float correctedStallAngleLow, float currentFlapAngle)
    {
