...
public static class AerodynamicUtils
{
    ...
    public static float DeltaLift(float liftSlope, float theta, float flapAngle)
    {
        return liftSlope * (1 - (theta - Mathf.Sin(theta)) / pi) *
               Mathf.Lerp(0.8f, 0.4f, (flapAngle * deg2Rad - 10) / 50) * flapAngle; // FlapEffectivenessCorrection(flapAngle)
    }
    ...
    public static float MaxHigh(float liftSlope, AerodynamicSurface config, float zeroLift, float deltaLift)
    {
        return liftSlope * (config.stallAngleHigh * deg2Rad - zeroLift) +
               deltaLift * Mathf.Clamp01(1 - 0.5f * (config.flapFraction - 0.1f) / 0.3f); // LiftCoefficientMaxFraction(config.flapFraction)
    }
    ...
    public static Vector3 CoefficientAtStall(float normalCoefficient, float effectiveAngle, float tangentialCoefficient)
    {
        return new Vector3(
