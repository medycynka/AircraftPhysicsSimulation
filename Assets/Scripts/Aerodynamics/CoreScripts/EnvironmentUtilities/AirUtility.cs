...
namespace Aerodynamics.CoreScripts.EnvironmentUtilities
{
    public static class AirUtility
    {
        ...
        public static float CalculateAirPressureAtAltitude(float altitude, float kelvinDegrees,
            float pressureAtSeaLevel, float seaLevel = 0f)
        {
            return pressureAtSeaLevel * Mathf.Exp((-gravitationForce * airMolarMass * (altitude - seaLevel)) /
                                                  (universalGasConstant * kelvinDegrees));
        }

        public static float CalculateSaturationVapor(float kelvinDegrees)
        {
            return 6.1078f * Mathf.Pow(10, (7.5f * kelvinDegrees) / (kelvinDegrees + 237.3f));
        }

        public static float CalculateSaturationVaporE(float celsiusDegrees)
        {
        ...
        {
            return relativeHumility * CalculateSaturationVaporE(celsiusDegrees);
        }

        public static float CalculateAirDensity(float celsiusDegrees, float relativeHumility, float altitude,
            float pressureAtSeaLevel, float seaLevel = 0f)
        {
            float kelvinDegrees = CelsiusesToKelvins(celsiusDegrees);
            float airPressure = CalculateAirPressureAtAltitude(altitude, kelvinDegrees, pressureAtSeaLevel, seaLevel);
            float vaporPressure = CalculateVaporPressure(celsiusDegrees, relativeHumility);
            float deltaPressure = airPressure - vaporPressure;

            return (deltaPressure / (dryAirGasConstant * kelvinDegrees)) +
                   (vaporPressure / (waterGasConstant * kelvinDegrees));
        }
    }
}