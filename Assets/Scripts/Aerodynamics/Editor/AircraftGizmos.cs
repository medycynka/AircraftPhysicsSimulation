...
namespace Aerodynamics.Editor
{
    public static class AircraftGizmos
    {
        ...
        public static void AircraftPhysicsGizmos(PhysicsManager phys, GizmoType gizmoType)
        {
            AircraftPhysicsDisplaySettings settings = AircraftPhysicsDisplaySettings.instance;
            Vector3 weight = phys.GetComponent<AirplaneController>().mass * Physics.gravity;
        ...
        public static void SurfaceGizmos(AerodynamicSurfaceManager surf, GizmoType gizmoType)
        {
            ...
            if (settings.showForces)
            {
                ...
                DrawForces(surf.transform, surf.currentLift * scale, surf.currentDrag * scale,
                    surf.currentTorque * scale);
            }
        }

        private static void DrawSurface(Transform transform, AerodynamicSurface config, float flapAngle,
        ...
        private static void DrawThinArrow(Vector3 position, Vector3 vector, Color color, float headSize, float width)
        {
            ...
            Handles.DrawAAPolyLine(width, position + vector,
                position + vector - vn * headSize + cross * headSize * 0.25f);
            Handles.DrawAAPolyLine(width, position + vector,
                position + vector - vn * headSize - cross * headSize * 0.25f);
        }
