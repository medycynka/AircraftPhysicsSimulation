...
{
    [Range(1, 15)] public float thrust = 5;
    ...
    private void FixedUpdate()
    {
        ...
        Vector3 velocityPrediction = PredictVelocity(forceAndTorqueThisFrame.p
            + transform.forward * (_thrust * _thrustPercent) + Physics.gravity * _rb.mass);
        Vector3 angularVelocityPrediction = PredictAngularVelocity(forceAndTorqueThisFrame.q);

        PowerTorqueVector3 forceAndTorquePrediction = 
    ...
    private PowerTorqueVector3 CalculateAerodynamicForces(Vector3 velocity, Vector3 angularVelocity, Vector3 wind, float airDensity, Vector3 centerOfMass)
    {
